 ...
 */
 ...
 const client = new Geocoding({key: 'C0PP56BGlxvP80NvqGxz2A5rRfo3AJQD'})
...
const getReverseGeocodingData = (lat, lng) => {
  return new Promise((resolve, reject) => {
   ...
   /// new
  // console.log(`from here ${lat} `)
    client.reverse(lat, lng)  //lat, lng
  .then(result => {
    console.log("first")
    console.log(`Result from reverse ${result}`)
    resolve(result.addressString); //single line address, ie: "1555 Blake St., Denver, CO 80204"
...
function getGeocodingData(addr) {
  ...
  .then(result => {
    console.log(result);
     // returns a GeoJson point of the first match
